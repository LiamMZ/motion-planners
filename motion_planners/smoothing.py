...
def smooth_path_force_aware(path, extend, collision, torque_fn, dynam_fn, distance_fn=None, iterations=50, max_time=INF, verbose=False):
    # TODO: makes an assumption on the distance metric
    # TODO: smooth until convergence
    assert (iterations < INF) or (max_time < INF)
    start_time = time.time()
    ...
    for iteration in irange(iterations):
        ...
        if all(not collision(q) for q in traverse(extend(point1, point2))):
            waypoints = new_waypoints
    #return waypoints
    new_path, _,new_vels, new_accels = dynam_fn(waypoints)
    if new_path is None:
        return None, None, None
    for i in range(len(new_path)):
        if not torque_fn(path[i], velocities=new_vels[i], accelerations=new_accels[i]):
            return None, None, None
    return path, new_vels, new_accels
